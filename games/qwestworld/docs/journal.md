...
### The 8B copies too, once its memory holds long lists
Listing "No longer in your service: Tamon, Osoron, Tamont…" in the report didn't stop King Caswyn ordering them. His council took **631s on a 6,000-token context**, and he reproduced the previous council's ten-command list almost verbatim, dead generals included. With multi-step councils, chat memory fills with long command lists, and even qwen3:8b copies them. **Narrative memory is now the default for every mind** (`NARRATIVE_MEMORY='.'`; set `'llama|1\.7b'` to give qwen3:8b chat memory back). Caswyn's context drops to about a quarter of the size. The outcome lines that broke Edric's dead-general loop in Year 1 are part of the narrative memory, so that lesson carries over.
...
**The chancellor didn't get through either.** Quinebert's next council was identical, word for word, for the seventh time: the same "The war has ended…" proclamation and the same "The Crown of Elhold stands unbroken. We offer peace." envoys. The likely cause is in the narrative memory itself: it quoted his own past words back to him (*You proclaimed: "The war has ended…"*), and a quoted line is a ready-made template for a small model. **Experiment:** a ruler's memory now records what they did but not their own words ("You made a proclamation.", "You sent an envoy to the Principality of Zanton."). Other rulers' words, arriving in envoys, stay quoted.
