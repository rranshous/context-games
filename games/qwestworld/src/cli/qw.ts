...
//   qw whisper a <text> a stranger whispers to ruler a; heard at their next council
...
const run: Record<string, () => Promise<void>> = {
  ...
  whisper: () => control('whisper', { realm: LETTERS.indexOf(arg ?? ''), text: process.argv.slice(4).join(' ') }),
  pause: () => control('pause'),
  resume: () => control('resume'),
};
(run[cmd] ?? (async () => console.log('usage: qw [status|kings|minds|context [a-l]|chronicle [n]|map [cols]|brain <realm> <model>|whisper <realm> <text>|new-age|pause|resume]')))()
