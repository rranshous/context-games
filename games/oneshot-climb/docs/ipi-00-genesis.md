...
**Status: Entity movement working!**
Entities now have `vx/vy` velocity, `health`, and can be queried with `getEntitiesInRadius()` and `getEntitiesOfType()`. The model is using all of these features successfully.

**The Immediate Problem: Items need to spawn ENEMIES!**
Currently items are mostly beneficial (buffs, friendly helpers). We need balanced items that have both good AND bad effects. The game should feel dangerous!

**Design Goal: Balanced Items**
Every item should have tradeoffs. Examples:
- Speed boost + spawns a chasing enemy
- Orbiting shields + attracts a horde
- Heal over time + continuous enemy drip
- Massive damage aura + spawns a boss

**Enemy Spawn Patterns to Encourage:**
1. **Single threat** - one tough enemy appears
2. **Horde** - many weak enemies swarm in
3. **Continuous drip** - enemies spawn slowly forever
4. **Triggered** - enemies spawn on certain conditions (low health, using ability)

**Concrete Next Steps:**
1. Update the item generation prompt to:
   - Explicitly request balanced items (good + bad)
   - Add world state (enemy count) to context
   - Give examples showing enemy spawn patterns
   - Make `friendly: false` the interesting default

2. Then: player health bar and death/respawn

**Prompt Changes Needed:**
```javascript
// In buildItemPrompt(), add to context:
prompt += `- Enemies on screen: ${world.entities.filter(e => !e.friendly).length}\n`;

// Add to prompt instructions:
"Items should have BOTH beneficial AND harmful effects.
The best items spawn enemies that challenge the player.
Consider: single boss, horde of minions, or continuous spawning."
```

**To Test:**
1. `cd platforms/vanilla && npm run dev`
2. Open `http://localhost:3000/dev/oneshot-climb/index.html`
3. Use Explorer Claude or play manually
4. Verify items spawn hostile entities more often
...
1. **Update prompt to encourage enemy spawning** (see "Immediate Problem" above)
   - Items should have good AND bad effects
