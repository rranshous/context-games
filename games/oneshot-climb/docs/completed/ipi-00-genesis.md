...
- Y/A/B buttons: Active abilities (assigned by items)
- Start: Pause? (later)

**Keyboard equivalent:**
- WASD/Arrows: Move
- W/Space: Jump
- E: Interact with forge
- 1/2/3: Active abilities (Y/A/B)
...
- [x] Journal tracks item descriptions (visible in left panel)
- [x] Next forge uses journal in prompt

## Progress

*Status: M1-M6 complete! Movement helpers, active abilities, and journal UI all implemented. Ready for gameplay exploration.*
...
**Current Status: ✅ M1-M6 Complete + Movement Helpers + Active Abilities!**
- Items generate with mixed effects (good + bad tradeoffs)
- Enemies spawn with automatic movement behaviors (chase, orbit, patrol, wander, flee)
- Player health system working (damage, heal, i-frames, respawn)
- Active abilities on Y/A/B buttons with cooldowns
- Persistent item panel with journal of previous items
- Full gamepad support (no mouse needed)
...
- `world.randomPoint()`, `world.getPlayerPos()`, `world.getForgePos()`

*Entity API:*
- `entity.x`, `entity.y`, `entity.vx`, `entity.vy`, `entity.health`
- `entity.setMovement(type, options)` - chase/orbit/patrol/wander/flee

*Player API:*
- `player.heal()`, `player.damage()`, `player.addSpeed()`, `player.addJumpPower()`
- `player.onButtonPress(button, callback, { cooldown, name })` - active abilities

*Controls:*
- Gamepad: Stick=move, Stick-up=jump, X=forge, Y/A/B=abilities
...
**Key Code Locations:**
- Item prompt: `ITEM_GENERATION_PROMPT` around line ~580
- World API: `world` object around line ~1380
- Player API: `playerAPI` object around line ~1540
- Movement behaviors: `updateWorld()` around line ~1700
- Explorer Claude: around line ~2350
...
### 2026-02-01: UI Improvements & Movement/Ability Systems

**Item Panel Made Persistent:**
- Removed collapse/toggle behavior - panel always visible
- Renamed header to "Items"
- Added Journal section below current item
  - Shows previous items with sprite thumbnails
  - Truncated descriptions (2-line clamp)
  - Items move to journal when new one is collected
- Full gamepad control - no mouse needed

**Movement Helper APIs Implemented:**

Added `entity.setMovement(type, options)` for automatic movement patterns:

| Behavior | Options | Description |
|----------|---------|-------------|
| `chase` | `speed` | Follows the player |
| `orbit` | `radius`, `period`, `startAngle` | Circles around player |
| `patrol` | `x1`, `y1`, `x2`, `y2`, `speed` | Moves between two points |
| `wander` | `speed`, `interval` | Random movement |
| `flee` | `speed`, `fleeRadius` | Runs from player |

Added helper methods:
- `world.randomPoint()` - random {x, y} in play area
- `world.getPlayerPos()` - player center position
- `world.getForgePos()` - forge center position

**Active Ability System Implemented:**

Added `player.onButtonPress(button, callback, options)`:
- Buttons: `'Y'`, `'A'`, `'B'` (gamepad) / `1`, `2`, `3` (keyboard)
- Options: `{ cooldown: seconds, name: 'AbilityName' }`
- Abilities appear in UI below health bar with cooldown indicators
- Edge-triggered (fires once per press, not while held)

**Control Scheme Updated:**
- X = Forge interaction (was already X, just clarified)
- Y/A/B = All available for abilities
- Jump moved to stick-up (allows A button for abilities)

**Prompt Updated:**
- Added 4 examples showing new APIs
- Example 1: Guardian Orbs (orbit movement)
- Example 2: Cursed Totem (chase movement)
- Example 3: Sentry Stones (patrol movement)
- Example 4: Flame Gauntlet (active ability on Y)

**Effect Parser Updated:**
- Detects `player.onButtonPress()` → shows `[Y] AbilityName`
- Detects `setMovement()` → shows "Movement: chase, orbit, etc."

**Code Locations:**
- Movement behaviors: `updateWorld()` around line ~1700
- Movement helpers on world: around line ~1460
- Ability system on playerAPI: around line ~1580
- Ability triggering in game loop: around line ~2200
- Journal CSS: lines ~350-395
- Journal JS in itemPanel: around line ~2230

---

## Design Observations

### 1. Movement/Routing Helpers ✅ IMPLEMENTED

Entities can now use `setMovement()` for common patterns:
- `entity.setMovement('chase', { speed: 100 })`
- `entity.setMovement('orbit', { radius: 60, period: 2 })`
- `entity.setMovement('patrol', { x1, y1, x2, y2, speed })`
- `entity.setMovement('wander', { speed: 50, interval: 1.5 })`
- `entity.setMovement('flee', { speed: 120, fleeRadius: 150 })`

The AI no longer needs to implement its own movement in `onUpdate` - just call `setMovement()` after spawning.

### 2. Button Press Hooks ✅ IMPLEMENTED

Items can now assign active abilities:
- `player.onButtonPress('Y', callback, { cooldown: 2, name: 'Fireball' })`
- Three slots: Y, A, B (gamepad) / 1, 2, 3 (keyboard)
- Cooldown system with UI indicators
- Jump moved to stick-up to free up A button
