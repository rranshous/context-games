...
function buildReef(scene2, map2, tileSize, gradientMap2) {
  ...
  const floorSize = Math.max(map2.width, map2.height) * tileSize + 20;
...
function updateSquid(squid2, dt, t, map2, tileSize) {
  ...
  const newX = squid2.group.position.x + moveDir.x * MOVE_SPEED * dt;
  const newZ = squid2.group.position.z + moveDir.z * MOVE_SPEED * dt;
